...
int main(int argc, char *argv[]){
    ...
    std::string intermediate_save_path = "";
    if (!show_ui && argc > 2) {
        intermediate_save_path = argv[2];
    }
    ...
    for (int generation=0; generation<n_generations; generation++){
        ...
        for (int i=0; i<n_cross; i++){
            ...
            if (!intermediate_save_path.empty()) {
            // 1. ??????????????
            std::shared_ptr<LabelingIndividual> best_gen_indiv = archive.getIndiv(0);
            
            // 2. ???? Charts ? Turning Points ????(????)
            best_gen_indiv->updateChartsAndTPs(true);

            // 3. ???????? (Deformed Vertices)
            // ???????? Labeling ???????
            Eigen::MatrixXd gen_V = qle->computeDeformedV(best_gen_indiv->getLabeling());

            // 4. ????? (??: .../iter_00.obj)
            std::string filename = intermediate_save_path + "/iter_" + std::to_string(generation) + ".obj";
            
            // 5. ??? OBJ
            igl::writeOBJ(filename, gen_V, F);
            
            std::cout << "Saved generation " << generation << " to " << filename << std::endl;
            }
